...
io.on('connection', (socket) => {
    ...
    socket.on('join_multiplayer', (data) => {
        players[socket.id] = {
            ...
            x: Math.random() * 3000, y: 0 // Spawn in sky
        };
    ...
    socket.on('combat_kill', (victimId) => {
        ...
        if(killer && victim) {
            ...
            // Drop scrap coins at world coords
            for(let i=0; i<4; i++) {
                mapCoins.push({ id: Date.now()+i, x: victim.x + (Math.random()*100-50), y: victim.y, value: 2 });
            }
...
const PORT = 3000;
server.listen(PORT, () => {
    console.log(`Sawblade Server running on port ${PORT}`);
});
